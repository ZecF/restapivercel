...
const PRESET_EFFECTS = {
  ...
  'glitch': 'https://en.ephoto360.com/create-a-glitch-cyberpunk-text-effect-online-787.html',
...
async function generate(effectNameOrUrl, texts) {
  ...
  try {
    ...
    if (!token) throw new Error('Gagal mengekstrak form token Ephoto.');
    ...
    if (!jsonMatch) throw new Error('Gagal mendapatkan signature payload Ephoto.');
    ...
    if (!createRes.data || !createRes.data.success) throw new Error('Gagal memproses gambar pada server Ephoto360.');
...
module.exports = { ephotoMaker: generate };
